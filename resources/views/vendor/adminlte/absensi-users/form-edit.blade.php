...
<form action="{{ route('admin.absensi_user.update', $absensi_user) }}" class="form-update-absensi_user" method="POST" enctype="multipart/form-data">
    @csrf
	@method('patch')
				...
				<div class="card-body">
							...
							<div class="form-group p-2 mb-1">
								...
								<select required="required" name="id_unit" id="input-id_unit"
									...
									@foreach (\App\Models\UnitModel::orderBy('code', 'asc')->get() as $i => $s)
										<option value="{{ $s->id }}" @selected( $s->id == $absensi_user->id_unit)>{{ $s->nama }}</option>
									@endforeach
								</select>
							</div>
						</div>
					...
					<div class="row">
						<div class="col-sm-12 p-0 px-1">
							<div class="form-group p-2 mb-1">
								...
								<input required="required" type="text" name="nama" value="{{ $absensi_user->nama }}"
					...
					<div class="row">
						<div class="col-sm-12 p-0 px-1">
							<div class="form-group p-2 mb-1">
								<label>Jabatan</label>
								<input required="required" type="text" name="jabatan" value="{{ $absensi_user->jabatan }}"
								class="form-control border py-0 text-italic-0 form-control-sm"
								placeholder="Jabatan" />
							</div>
						</div>
						...
						<div class="col-sm-12">
							<div class="form-group p-2 mb-1">
								...
								<select required="required" name="jenis_kelamin"
									...
									<option value="Laki-Laki" @selected($absensi_user->jenis_kelamin == "Laki-Laki")>Laki-Laki</option>
									<option value="Perempuan" @selected($absensi_user->jenis_kelamin == "Perempuan")>Perempuan</option>
								</select>
							</div>
						</div>
					</div>
					...
					<div class="row">
						<div class="col-sm-12">
							<div class="form-group p-2 mb-1">
								...
								<img src="{{ asset(Storage::url($absensi_user->photo)) }}" style="height: 150px;"/>
								@endif
							</div>
						</div>
					</div>
...
<script>
	...
	$(document).ready(function(){
		...
		$("body").on("submit",".form-update-absensi_user",function(e){
			e.preventDefault();
			var id = $(this).data("id");
			var url = "{{ route('admin.absensi_user.update', $absensi_user)}}";
			var data = new FormData($(".form-update-absensi_user")[0]);
