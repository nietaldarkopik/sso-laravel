...
<form action="{{ route('admin.absensi_device_user.update', $absensi_device_user) }}" class="form-edit-absensi_device_user" method="POST" enctype="multipart/form-data" data-id="{{ $absensi_device_user->id }}">
    @csrf
	@method('patch')
    ...
    <div class="row mb-3">
        ...
        <div class="col-xs-12 col-sm-12 col-lg-12 mb-3">
							...
							<div class="form-group p-2 mb-1">
								...
								<select required="required" name="id_device" id="input-id_device"
									...
									@foreach (\App\Models\AbsensiDeviceModel::get() as $i => $s)
										<option value="{{ $s->id }}" @selected($s->id == $absensi_device_user->id_device)>{{ $s->device_no }} | {{ $s->device_name }} | {{ $s->location }}</option>
									@endforeach
								</select>
							</div>
						</div>
							...
							<div class="form-group p-2 mb-1">
								...
								<select required="required" name="id_user" id="input-id_user"
									...
									@foreach (\App\Models\AbsensiUserModel::get() as $i => $s)
										<option value="{{ $s->id }}" @selected($s->id == $absensi_device_user->id_user)>{{ $s->nama }}</option>
									@endforeach
								</select>
							</div>
						</div>
					...
					<div class="row">
						<div class="col-sm-12 p-0 px-1">
							<div class="form-group p-2 mb-1">
								...
								<input required="required" type="text" name="user_no" value="{{ $absensi_device_user->user_no }}"
					...
					<div class="row">
						<div class="col-sm-12 p-0 px-1">
							<div class="form-group p-2 mb-1">
								...
								<input required="required" type="text" name="device_user_name" value="{{ $absensi_device_user->device_user_name }}"
								class="form-control border py-0 text-italic-0 form-control-sm"
			...
			<div class="col-xs-12 col-sm-12 col-lg-12 mb-1 text-center">
				<button class="btn btn-primary btn-save-absensi_device_user">
...
<script>
	...
	$(document).ready(function(){
		...
		$("body").on("submit",".form-edit-absensi_device_user",function(e){
			e.preventDefault();
			$(".btn-save-absensi_device_user").prop("disabled",true);
			var id = $(this).data("id");
			var url = "{{ route('admin.absensi_device_user.update',['absensi_device_user' => 'xxx'])}}";
			//var data = new FormData($(".form-edit-absensi_device_user")[0]);
			var data = $(".form-edit-absensi_device_user").serializeArray();
			...
			$.ajax({
				...
				success:function(msg){
					...
					var id_absensi_device_user = (!data.id)?0:data.id;
					
					if(!id_absensi_device_user){
					...
					$(".btn-save-absensi_device_user").prop("disabled",false);
				},
		...
		$("body").on("click",".btn-save-absensi_device_user",function(){
		});
