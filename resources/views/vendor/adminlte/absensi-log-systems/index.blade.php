...
@section('title', 'Data Log Sistem')

@section('content_header')
    <h1 class="m-0 text-dark">Data Log Sistem</h1>
@stop

@section('content')
    <div class="card">
        <div class="card-header">
            <h2 class="card-title fw-bold fs-4">Data Log Sistem</h2>
...
<script>
    ...
    <script>
        ...
        $('#search').on('click', function(e) {
            ...
            window.LaravelDataTables["absensi-log-system-table"].table()
                                                            .ajax
                                                            .url('{{ route('admin.absensi_log.index') }}?start_date='+start_date+'&start_date=' + start_date + '&end_date=' + end_date)
                                                            .load();
            /* window.LaravelDataTables["absensi-log-system-table"].table().ajax.params ={
                start_date: start_date,
                end_date: end_date
            };
            console.log(window.LaravelDataTables["absensi-log-system-table"].table().ajax.params);
            window.LaravelDataTables["absensi-log-system-table"].table().ajax.data({
                start_date: start_date,
                end_date: end_date
            }).load();
            
             //window.LaravelDataTables["absensi-log-system-table"].column(8).search(tahun_siteplan ? tahun_siteplan : '', false, false);
             window.LaravelDataTables["absensi-log-system-table"].table().ajax.reload(); //draw(); */
        });
