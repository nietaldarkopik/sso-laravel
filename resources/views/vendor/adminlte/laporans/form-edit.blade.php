...
<form action="{{ route('admin.laporan.update', $pengajuan->id) }}" class="form-create-pengajuan" method="POST" enctype="multipart/form-data" data-id="{{ $pengajuan->id }}">
    @csrf
							...
							<div class="form-group p-2 mb-1">
								...
								<select required="required" name="id_sop" id="input-id_sop"
									...
									@foreach (\App\Models\SopModel::orderBy('sop', 'asc')->get() as $i => $s)
										<option value="{{ $s->id }}" @selected($s->id == $pengajuan?->id_sop)>{{ $s->kode }} - {{ $s->sop }}</option>
									@endforeach
								</select>
							</div>
						</div>
							...
							<div class="form-group p-2 mb-1">
								...
								<select required="required" name="id_unit" id="input-id_unit"
									...
									@foreach (\App\Models\UnitModel::orderBy('code', 'asc')->get() as $i => $s)
										<option value="{{ $s->id }}" @selected($s->id == $pengajuan?->id_unit)>{{ $s->code }} - {{ $s->nama }}</option>
									@endforeach
								</select>
							</div>
						</div>
					...
					<div class="row">
						<div class="col-sm-12 p-0 px-1">
							<div class="form-group p-2 mb-1">
								...
								<input required="required" type="text" name="judul" value="{{ $pengajuan?->judul }}"
					...
					<div class="row">
						<div class="col-lg-6 p-0 px-1">
							<div class="form-group p-2 mb-1">
								...
								<input required="required" type="number" name="nominal" value="{{ $pengajuan?->nominal }}"
								class="form-control border py-0 text-italic-0 form-control-sm"
						...
						<div class="col-lg-6">
							<div class="form-group p-2 mb-1">
								<label>Tanggal</label>
								<input type="text" name="tgl_pengajuan" value="{{ $pengajuan?->tgl_pengajuan}}"
						...
						<div class="col-sm-12">
							<div class="form-group p-2 mb-1">
								...
								<textarea type="text" name="keterangan" rows="5"
									class="form-control border py-0 text-italic-0 form-control-sm" placeholder="Keterangan">{{$pengajuan?->keterangan}}</textarea>
							</div>
						</div>
					</div>
						...
						<div class="col-sm-12">
							<div class="form-group p-2 mb-1">
								...
								<select required="required" name="status"
									...
									@foreach(App\Models\StatusLaporanModel::where('grup_form',1)->get() as $i => $s)
									<option value="{{$s->kode}}" @selected($s->kode == $pengajuan?->status) style="border-color: {{$s->warna_bg}} !important; background-color: {{$s->warna_bg}} !important; color: {{$s->warna_text}} !important;">{{$s->judul}}</option>
									@endforeach
					...
					<div class="row">
						<div class="col-md-12 dokumen-container p-2">
							<div class="form-group">
								<label for="dropzone-document">Upload Dokumen Laporan</label>
				...
				<div class="card-body">
					...
					<div class="row">
						<div class="col-md-12 template-container p-2">
							<div class="alert alert-lg alert-info">Silahkan Pilih Jenis Laporan terlebih dahulu</div>
