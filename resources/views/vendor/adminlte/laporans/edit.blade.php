...
@section('content')
    <div class="card col-md-12">
        <div class="card-header">
            <h2 class="card-title fw-bold fs-4">Edit Laporan</h2>
            <div class="card-tools">
                @can('admin.laporan.index')
                    <a class="btn btn-sm btn-primary" href="{{ route('admin.laporan.index') }}">
        ...
        <div class="card-body">
            @include('vendor.adminlte.laporans.form-edit',['pengajuans'])
        </div>
    </div>
...
<script>
	...
	$(document).ready(function(){
		...
		function id_shop_changed(){
        	...
        	var url = "{{ route('admin.laporan.getStepSop',['sop_id' => 'xx'])}}";
...
<script>
	$(document).ready(function(){
		...
		$('.dropzone-document').dropzone({
			url: '{{ route('admin.laporan.uploadDokumen',['pengajuan' => $pengajuan->id]) }}',
			maxFilesize: 512, // MB
			dataType: "json",
			addRemoveLinks: true,
			...
			success: function(file, response) {
				...
				toastr.success('Halaman akan diarahkan ke Tracking Laporan', 'Data berhasil dikirim', {
					timeOut: 5000,
					//positionClass: "toast-top-right",
					progressBar: true,
                    onShown: function() {
		...
		$("body").on("submit",".form-create-pengajuan",function(e){
			...
			var kode = $(this).val();
			var id = $(this).data("id");
			var url = "{{ route('admin.laporan.update',['pengajuan' => $pengajuan->id])}}";
				...
				success:function(msg){
					...
					if(!id_pengajuan){
						...
						if (myDropzone.getQueuedFiles().length === 0) {
							...
							var url = '{{ route('admin.laporan.uploadDokumen',['pengajuan' => 'xxx']) }}';
