...
<form action="{{ route('admin.absensi_attendence_log.update', $absensi_attendence) }}" class="form-edit-absensi_attendence" method="POST" enctype="multipart/form-data" data-id="{{ $absensi_attendence->id }}">
    @csrf
	@method('patch')
...
<script>
	...
	$(document).ready(function(){
		...
		$("body").on("submit",".form-edit-absensi_attendence",function(e){
			...
			var id = $(this).data("id");
			var url = "{{ route('admin.absensi_attendence_log.update',['absensi_attendence' => 'xxx'])}}";
				...
				success:function(msg){
					...
					if(!id_absensi_attendence){
						...
						toastr.success('Halaman akan diarahkan ke Data Karyawan', 'Data berhasil dikirim', {
							...
							onShown: function() {
								setTimeout(() => {
									window.location.href = "{{ route('admin.absensi_attendence_log.index')}}";							
								}, 3000);
							}
						});
					}
