...
class TindakansDataTable extends DataTable
{
    ...
    public function dataTable(QueryBuilder $query, Request $request): EloquentDataTable
    {
        return (new EloquentDataTable($query))
            ->addColumn('action', 'vendor.adminlte.tindakans.datatables_action')
    ...
    public function query(PengajuanModel $model,Request $request): QueryBuilder
    {
		...
		$status_perlu_tindakan = \App\Models\StatusPengajuanModel::where('grup',2)->get()->pluck('kode');
		$status_belum_perlu_tindakan = \App\Models\StatusPengajuanModel::where('grup',2)->get()->pluck('kode');
		$user_id = Auth::user()->id;
        return $q->whereHas('progress',function($query) use ($user_id,$status_perlu_tindakan){
			$query->where(function($query2) use ($user_id,$status_perlu_tindakan) {
				$query2->where('id_user',$user_id);
				/* $query2->whereIn('status',$status_perlu_tindakan); */
			});
		})->where(function($query) use ($status_perlu_tindakan,$status_belum_perlu_tindakan,$user_id){
			$query->where('id_user','!=',$user_id);
			$query->whereIn('status',$status_belum_perlu_tindakan);
		})->orderBy('id','desc');
